...
import { useAdminAlertConfig, useUpdateAlertConfig } from "@/hooks/use-backend";
import { toast } from "sonner";

export default function AdminAlerts() {
  const { data: remoteConfig, isLoading } = useAdminAlertConfig();
  ...
  if (remoteConfig && !localConfig && !isLoading) {
  ...
  if (isLoading) return <div className="p-8 text-center text-muted-foreground">Loading config...</div>;
