...
import { Shield, ShieldCheck, Smartphone, Copy, Eye, EyeOff } from "lucide-react";
...
export default function Security() {
  ...
  // Mock TOTP secret for demo
  const [mockSecret] = useState("JBSWY3DPEHPK3PXP");
  const [mockBackupCodes] = useState(["abc123", "def456", "ghi789", "jkl012", "mno345"]);

  // 2FA logic temporarily neutralized or linked to backend
  const { data: totpData, isLoading } = useQuery({
    queryKey: ["totp", user?.id],
    enabled: !!user?.id,
    queryFn: async () => {
      // Mocked for now to remove Supabase dependency
      return { is_enabled: false, backup_codes: [] };
    },
  });
